...
// int check_syn(cmu_socket_t *sock, uint32_t seq) {
//   int result;
//   while (pthread_mutex_lock(&(sock->window.ack_lock)) != 0)
//     ;
//   if (sock->window.last_ack_received > seq)
...
void handle_message(cmu_socket_t *sock, char *pkt) {
  ...
  case ACK_FLAG_MASK:
    ...
    break;
  case SYN_FLAG_MASK: 
    if (sock->type == 1) { //send SYN+ACK if initiator
      seq = get_seq(pkt);
      synackPkt = create_packet_buf(sock->my_port, ntohs(sock->conn.sin_port), seq,
                              seq + 1, DEFAULT_HEADER_LEN, DEFAULT_HEADER_LEN,
                              SYN_FLAG_MASK | ACK_FLAG_MASK, 1, 0, NULL, NULL, 0);
      sendto(sock->socket, synackPkt, DEFAULT_HEADER_LEN, 0,
            (struct sockaddr *)&(sock->conn), conn_len);
      free(synackPkt);
    }
    break;
  case SYN_FLAG_MASK | ACK_FLAG_MASK: //Send final ACK (handshake)
  case FIN_FLAG_MASK:
    if (get_ack(pkt) > sock->window.last_ack_received)
      sock->window.last_ack_received = get_ack(pkt);
      seq = get_seq(pkt);
...
void check_for_data(cmu_socket_t *sock, int flags) {
  ...
  if (len >= DEFAULT_HEADER_LEN) {
    // char str[20] = "check data";
    // printf("check_for_data %s \n" , str);
    plen = get_plen(hdr);
    pkt = malloc(plen);
    while (buf_size < plen) {
